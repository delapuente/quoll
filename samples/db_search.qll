...
def db_oracle(marked_qubit: Qubit, db_register: QRegister) -> Unit[Adj + Ctl]:
  Controlled[X](db_register, marked_qubit)


def uniform_superposition_oracle(db_register: QRegister) -> Unit[Adj + Ctl]:
  map(H, db_register)


def state_preparation_oracle(marked_qubit: Qubit, db_register: QRegister) -> Unit[Adj + Ctl]:
  uniform_superposition_oracle(db_register)
  db_oracle(marked_qubit, db_register)


def reflect_marked(marked_qubit: Qubit) -> Unit[Adj + Ctl]:
  import math
...
def reflect_start(marked_qubit: Qubit, db_register: QRegister) -> Unit:
...
def quantum_search(iterations: int, marked_qubit: Qubit, db_register: QRegister) -> Unit:
  state_preparation_oracle(marked_qubit, db_register)
...
def state_preparation_oracle_test() -> Unit:
  for db_size in range(6):
    with reset(allocate(1, db_size)) as (marked_qubit, db_register):
      state_preparation_oracle(marked_qubit, db_register)
...
def grover_hard_coded_test() -> Unit:
